...
from feature_extractor import build_feature_vector, validate_against_model_metadata

BASE_DIR = Path(__file__).resolve().parent
MODEL_PATH = BASE_DIR / "m2_mobile_model.joblib"
METADATA_PATH = BASE_DIR / "m2_mobile_model_metadata.json"
...
def load_artifacts():
    ...
    if _model is None:
        if not MODEL_PATH.exists():
            raise FileNotFoundError(f"Missing model artifact: {MODEL_PATH.resolve()}")
...
def _base_response(
    ...
    *,
    ...
    return {
        ...
        "reason": reason,
        "model_version": metadata.get("model_version", "M2_mobile_screen_location_v1"),
...
def score_participant_events(
    ...
    participant_id: str,
    window_end_date: str,
    normalization_rows: Optional[pd.DataFrame | Iterable[Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    ...
    recommended = bool(metadata.get("fusion_eligible_recommendation", False))
